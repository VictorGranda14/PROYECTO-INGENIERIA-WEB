...
export class HomePage implements OnInit {
  ...
  cargarDatos() {
    this.http.get('assets/sensores.Json').subscribe((data: any) => {
  ...
  mostrarDatos(datos: any) {
    ...
    if (sensorDataDiv) {
      ...
      for (const sensor in datos.sensores) {
        ...
        sensorElement.textContent = `${sensor.charAt(0).toUpperCase() + sensor.slice(1)}: ${detalles.valor} ${detalles.unidad}`;
